...
class OpenIdBaseClient(OpenIdProxyClient):
    ...
    def __enter__(self):
        """
        Return object that is to be used while entering context manager.

        Returns:
            self (BodhiClient): The calling object.
        """
        return self

    def __exit__(self, type, value, traceback):
        """
        Perform clean up after task is completed by context manager.
        """
        self.close()

    def close(self):
        """
        Closes the session object generated by __init__() to prevent
        system resource warnings of open socket
        """
